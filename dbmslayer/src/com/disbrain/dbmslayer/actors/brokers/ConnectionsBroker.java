...
import com.disbrain.dbmslayer.messages.GetStatisticsRequest;
...
public class ConnectionsBroker extends UntypedActor {
    ...
    public static String getBrokerStats() {
        return String.format("Internal stats:\nCurrently taken: %d Get errors: %d Release errors: %d Total usage: %d\n",
                stats.getTaken(),
                stats.getTakeErrors(),
    ...
    public void onReceive(Object message) {
        ...
        do {
            if (message instanceof GetStatisticsRequest) {
                getSender().tell(connection_pool.getStatistics() + "\n" + getBrokerStats(), getSelf());
                return;
            }
            ...
            if (message instanceof GetDbmsConnectionRequest) {
                ...
                try {
                    ...
                    output = new GetDbmsConnectionReply(connection, ((GetDbmsConnectionRequest) message).connection_params);
                    stats.gotConnection();
            ...
            log.error("CONNECTION BROKER UNKNOWN MESSAGE DETECTED: " + message.getClass().getName());
